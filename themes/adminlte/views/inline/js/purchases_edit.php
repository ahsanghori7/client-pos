(function($){ 
    ...
    postatus,
    poshipping,
    popayment_term, poref, powarehouse, ponote, posupplier;
    let  
        product_discount = 0,
        order_discount = 0,
        sortedItems;
...
$('#posupplier').select2();
jQuery(function ($) {        
    jQuery(document).on("submit", "#purchase_form", function (e) {
 ...
 function add_purchase_item(item) {
    ...
    $(document).ready(function () {
        if(invoice && invoice !== "") {
            ...
            localStorage.setItem('posupplier', invoice.supplier_id);
            localStorage.setItem('poref', invoice.reference_no);
            localStorage.setItem('postatus', invoice.status);
            localStorage.setItem('ponote', invoice.note);
            localStorage.setItem('podiscount', invoice.order_discount_id);
            localStorage.setItem('potax2', invoice.order_tax_id);
            localStorage.setItem('poshipping', invoice.shipping);
            ...
            localStorage.setItem('poitems', inv_items);
        }
        ...
        let podate = localStorage.getItem('podate')
        if (podate) {
            $('#podate').val(podate);
        }else{
            $('#podate').val($('#purchase_date_fld').val());
        ...
        jQuery(document).on("click", "#edit_pruchase", function (e) {
            $(window).unbind('beforeunload');
