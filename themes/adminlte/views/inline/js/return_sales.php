    ...
    function loadReturnItems() {
        ...
        if (localStorage.getItem('reitems')) {
            ...
            $.each(reitems, function () {
                ...
                tr_html += '<td class="text-right"><input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' + row_no + '" value="' + item_cost + '"><input class="rucost" name="unit_cost[]" type="hidden" value="' + unit_cost + '"><input class="realucost" name="real_unit_cost[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right scost" id="scost_' + row_no + '">' + formatMoney(item.row.real_unit_price) + '</span></td>';
                ...
                if(item.row.remaining_qty == null || item.row.remaining_qty != 0){             
                tr_html += '<td><input class="form-control text-center ret_quantity" name="quantity[]" type="number" value="0" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_quantity[]" type="hidden" class="ret_quantity" value="' + quantity + '"></td>';
                }else{
                    tr_html += '<td>'+item.row.received+'</td>';
                }
                ...
                tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + item.row.subtotal + '</span></td>';
                newTr.html(tr_html);
