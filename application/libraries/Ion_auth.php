...
class Ion_auth
{
			...
			if ($user)
			{
				$data = array(
                    ...
                    $parse_data = array(
                        ...
                        'email' => $user->email,
                        'reset_password_link' => anchor('panel/login/reset_password/' . $user->forgotten_password_code, lang('reset_password')),
                        'site_link' => base_url(),
                        'site_name' => $Settings->title,
                        'logo' => '<img src="' . base_url() . 'assets/uploads/logos/' . $Settings->logo . '" alt="' . $Settings->title . '"/>'
                    );

                    $msg = file_get_contents(FCPATH .'themes/'. $Settings->theme . '/views/email_templates/forgot_password.html');
                    $message = $this->parser->parse_string($msg, $parse_data);
                    $message = $message . "<br>" . lang('reset_password_link_alt') . "<br>" . site_url('reset_password/' . $user->forgotten_password_code);
                    $subject = lang('email_forgotten_password_subject') . ' - ' . $Settings->title;

                    try {
                        if ($this->repairer->send_email($user->email, $subject, $message)) {
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			{
				$message = $this->load->view($this->theme . $this->config->item('email_templates', 'ion_auth').$this->config->item('email_forgot_password_complete', 'ion_auth'), $data, true);
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		else
		{
			...
			{
				$message = $this->load->view($this->theme . $this->config->item('email_templates', 'ion_auth').$this->config->item('email_activate', 'ion_auth'), $data, true);
