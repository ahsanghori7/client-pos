...
?>$(document).ready(function () {
        jQuery(document).on("click", "#pdf", function (event) {
            ...
            window.location.href = "<?=site_url('panel/reports/getProductsReport/pdf/?v=1'.$v)?>";
            return false;
        });
        jQuery(document).on("click", "#xls", function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('panel/reports/getProductsReport/0/xls/?v=1'.$v)?>";
            return false;
        });
    });
...
function formatProfit(x) {
    ...
    $(document).ready(function () {
        ...
        var oTable = $('#dynamic-table').dataTable({
            ...
            'sAjaxSource': '<?= site_url('panel/reports/getProductsReport?v=1' . $v) ?>',
